...
static const char* path = "\\c64\\pagefile.sys";
static FIL file;

bool init_vram() {
    logMsg((char *)"Create <SD-card>\\c64\\pagefile.sys");
    ...
    if (result == FR_OK) {
        ...
        if (result != FR_OK) {
            logMsg((char *)"Unable to init <SD-card>\\c64\\pagefile.sys");
            return false;
        }
    } else {
        logMsg((char *)"Unable to create <SD-card>\\c64\pagefile.sys");
        return false;
    }
    f_close(&file);
    result = f_open(&file, path, FA_READ | FA_WRITE);
    if (result != FR_OK) {
        logMsg((char *)"Unable to open <SD-card>\\c64\\pagefile.sys");
    }
